ToGrayMoyenneTask=function(opt_options) {
...
ToGrayMoyenneTask.prototype.process=function(imageData) {
  ...
  for (var i = r; i < imageData.height-r; i++){
      for (var j = r; j < imageData.width-r; j++) {
        ...
        for (var l=-r; l<r+1; l++){
          for (var m=-r; m<r+1; m++){
            val +=(pixels[w+l*imageData.width*4+m*4]+pixels[w+1+l*imageData.width*4+m*4]+pixels[w+2+l*imageData.width*4+m*4])/3
          }
        }
...
PartialGrayMoyenneTask=function(opt_options) {
...
PartialGrayMoyenneTask.prototype.process=function(imageData) {
  var pixels = imageData.data;
  var p = new Array();
  for (var i = 0; i < imageData.height*imageData.width*4; i++){
      p[i]=pixels[i];
