ToMinTask=function(opt_options) {
...
ToMinTask.prototype.process=function(imageData) {
  ...
  for (var i = r; i < imageData.height-r; i++){
      for (var j = r; j < imageData.width-r; j++) {
        for (var k=0; k<4; k++){
          ...
          for (var l=-r; l<r+1; l++){
            for (var m=-r; m<r+1; m++){
              if(val>pixels[w+k+l*imageData.width*4+m*4]){
...
PartialMinTask=function(opt_options) {
...
PartialMinTask.prototype.process=function(imageData) {
  var pixels = imageData.data;
  var p = new Array();
  for (var i = 0; i < imageData.height*imageData.width*4; i++){
      p[i]=pixels[i];
